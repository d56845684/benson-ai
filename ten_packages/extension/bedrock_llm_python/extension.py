...
class BedrockLLMExtension(AsyncLLMBaseExtension):
    ...
    def load_json_from_s3(self, bucket_name: str, object_key: str, 
                        region_name: str = "us-west-2") -> dict:
        """
        從 S3 下載 JSON 檔案並轉成 Python dict。

        Args:
            bucket_name (str): S3 Bucket 名稱
            object_key (str): S3 內的物件路徑
            aws_access_key_id (str, optional): AWS Access Key (可選)
            aws_secret_access_key (str, optional): AWS Secret Key (可選)
            aws_session_token (str, optional): AWS Session Token (可選)
            region_name (str, optional): AWS 區域，預設 us-east-1
    ...
    async def on_start(self, ten_env: AsyncTenEnv) -> None:
        ...
        try:
            ...
            s3_user_profile_dict = self.load_json_from_s3(bucket_name="emg-user-system-prompt", object_key=f"{self.config.user_id}.json")
            # user_full_name = s3_user_profile_dict['user_name']
            self.user_profile = s3_user_profile_dict['user_profile']
            self.config.greeting = s3_user_profile_dict['opening']
            self.prompt = s3_user_profile_dict['prompt']
    ...
    async def on_stop(self, ten_env: AsyncTenEnv) -> None:
        """Stop the extension."""
        conversation_post_process_bedrock_flow_ID = "OPRZM75ZAX"
        conversation_post_process_bedrock_flow_AliasID = "F1S2L0CTBF"
        user_id = self.config.user_id
        session_id = self.uuid
        conversation_post_process_bedrock_flow_input = f"{user_id}/{session_id}.json"

        input_data = {
            "content": {
                "document": conversation_post_process_bedrock_flow_input
            },
            "nodeName": "FlowInputNode",
            "nodeOutputName": "document"
        }
        request_params = {
            "flowIdentifier": conversation_post_process_bedrock_flow_ID,
            "flowAliasIdentifier": conversation_post_process_bedrock_flow_AliasID,
            "inputs": [input_data],
            "enableTrace": True
        }
        response = self.bedrock_client.invoke_flow(**request_params)
    ...
    async def _call_nova_model(
        ...
        try:
            ...
            data = [{
                "content": {"document": json_loads},
                "nodeName": "FlowInputNode",
                "nodeOutputName": "document"
            }]
            # 童話結束 trigger
            # flow
            response = self.bedrock_client.invoke_flow(
                flowIdentifier="96LE2LLIJO",
                flowAliasIdentifier="H63E8ZGBIN",
                inputs=data
            )
            full_content = await self._process_bedrock_agent_response(response, start_time)
            # try:
            #     self.channel_name = await AsyncTenEnv.get_property_string(PROPERTY_CHANNEL_NAME)
            #     if self.channel_name:
            #         self.ten_env.log_info(f"await 頻道代號: {self.channel_name}, {input_text}")
            # except Exception as err:
            #     self.ten_env.log_error(
            #     f"GetProperty required {PROPERTY_CHANNEL_NAME} failed, err: {err}"
            # )
            # try:
            #     self.channel_name =  AsyncTenEnv.get_property_string(PROPERTY_CHANNEL_NAME)
            #     if self.channel_name:
            #         self.ten_env.log_info(f"頻道代號: {self.channel_name}, {input_text}")
            # except Exception as err:
            #     self.ten_env.log_error(
            #     f"GetProperty required {PROPERTY_CHANNEL_NAME} failed, err: {err}"
            # )
    ...
    async def _process_bedrock_agent_response(self, response: Dict, start_time: float):
        """Process streaming response from bedrock model."""
        sentence = ""
        full_content = ""
        first_sentence_sent = False
        # 取得模型的回應文字 (flow)
        for event in response.get("responseStream"):
            flow_event = event.get('flowOutputEvent')
            if flow_event:
                if flow_event and flow_event.get('nodeName') == 'lambda_model_response': ##bedrock flow
                    content = flow_event['content'].get('document')
                    ...
                    while True:
                        ...
                        self.ten_env.log_info(f"Processing sentence: [{sentence}]")
                        await self._send_text_data(sentence, False, Role.Assistant)     # 也許改成True
